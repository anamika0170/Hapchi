		...
		if (this.props.message.receiverType === CometChat.RECEIVER_TYPE.GROUP) {
			...
			avatar = (
				<div css={messageThumbnailStyle()} className='message__thumbnail'>
					<CometChatAvatar user={this.props.message.receiver} />
				</div>
			);
			...
			name = (
				<div css={nameWrapperStyle(avatar)} className='message__name__wrapper'>
					<span css={nameStyle(this.context)} className='message__name'>
						{this.props.message.receiver.name}
					</span>
				</div>
			);
		}
			...
			>
				<div css={messageWrapperStyle()} className='message__wrapper'>
					...
					<div css={messageDetailStyle()} className='message__details'>
						...
						<div style={{ fontSize: "12px", marginLeft: "73px", marginTop: "5px" }}>Hello <span style={{ color: "#014785", background: "#DAE7F0", borderRadius: "5px", padding: "5px", margin: "10px" }}>@{this.context.item.name}</span></div>

						<div
							css={messageTxtContainerStyle()}
							className='message__text__container'
						>
							<div style={{ marginLeft: "56px" }}>{messageText}</div>
						</div>
