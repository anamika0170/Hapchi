...
import { chatDetailStyle, chatHeaderStyle, chatNameStyle, chatSideBarBtnStyle, chatThumbnailStyle, chatUserStyle } from "../CometChatMessageHeader/style";
import { messageTimeStampStyle } from "../CometChatDeleteMessageBubble/style";
...
class CometChatSenderTextMessageBubble extends React.Component {
	...
	render() {
		...
		const messageDate = this.props.message.sentAt * 1000;
		return (
			<div
				...
				style={{ backgroundColor: "#EBF8FF", marginTop: "10px", width: '100%', display: 'block', justifyContent: 'flex-start' }}
			>
				...
				<div style={{ display: "flex", gap: "5px", alignItems: "center", marginTop: "10px" }}>
					...
					<div style={{ color: "#4C4B48", fontWeight: "bold" }}>{this.context.item.name}</div>
					<div style={{ color: "#aaa5a5", fontSize: "12px" }}>{dateFormat(messageDate, "shortTime")}</div>
				</div>

				<div style={{ fontSize: "12px", marginLeft: "70px", marginTop: "5px" }}>Hello <span style={{ color: "#014785", background: "#DAE7F0", borderRadius: "5px", padding: "5px", margin: "10px" }}>@{this.context.item.name}</span></div>
				<div
					css={messageWrapperStyle()}
					className='message__wrapper'
					ref={this.messageTextRef}
				>
					<div style={{ position: "relative", left: "53px" }}>{messageText}</div>
				</div>
