		...
		const messages = this.props.messages.map((message, key) => {
			...
			return (
				<React.Fragment key={key}>
							<Grid container spacing={1} style={{ marginLeft: '5px', marginRight: '10px', }} alignItems="center" justifyContent="center">
								<Grid item xs={5} >
									<Divider />
								</Grid>
								<Grid item xs={1.5} spacing={3} style={{position:'relative', top:'8px'}}>
									{dateSeparator}
								</Grid>
								<Grid item xs={5}>
									<Divider />
